...
class WorkflowActionRuleTask(models.Model):
    ...
    create_model = fields.Selection(selection_add=[('filing.document', "Filing")])

    def create_record(self, documents=None):
        rv = super(WorkflowActionRuleTask, self).create_record(documents=documents)
        if self.create_model == 'filing.document':
            new_obj = self.env[self.create_model].create({'name': "new filing from Documents"})
            task_action = {
